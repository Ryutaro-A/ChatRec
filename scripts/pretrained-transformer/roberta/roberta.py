...
sys.path.append("pretrained-transformer/roberta/util")
...
def run(args, use_large_model=False):
    ...
    print(f"roberta vocab dir: {args.vocab_dir}")
    ...
    large_config = transformers.RobertaConfig(
        attention_probs_dropout_prob=args.attention_dropout,
        bos_token_id=2,
        classifier_dropout=None,
        eos_token_id=3,
        hidden_act="gelu",
        hidden_dropout_prob=args.attention_dropout,
        hidden_size=1024,
        initializer_range=0.02,
    ...
    setattr(large_config, "num_labels", 1)

    # model
    if use_large_model:
        if args.use_pretrain_model:
            model = AutoModelForSequenceClassification.from_pretrained("nlp-waseda/roberta-large-japanese-seq512", config=large_config)
        else:
            model = RobertaForSequenceClassification(large_config)
    else:
        if args.use_pretrain_model:
            model = AutoModelForSequenceClassification.from_pretrained("nlp-waseda/roberta-base-japanese", config=config)
        else:
            model = RobertaForSequenceClassification(config)
    # embedingsを拡張
    model.roberta.resize_token_embeddings(len(train_dataset.tokenizer))
