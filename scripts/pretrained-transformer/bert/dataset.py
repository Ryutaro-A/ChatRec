...
class MyDataset(Dataset):
    def __init__(
        ...
        self.tokenizer = BertJapaneseTokenizer.from_pretrained(vocab_dir)
        self.pad_id = self.tokenizer.pad_token_id
        self.desc_id = self.tokenizer.vocab["[DESC]"]

        self.data_list = self.get_data_list(data_dir, filename_list, if_down_scale)
    ...
    def get_data_list(
        ...
        for filename in filename_list:
            ...
            for uttr_dict in chat_rec_jd["dialogue"]:
                ...
                encode_dict = self.tokenizer(uttr, add_special_tokens=True)
            ...
            for speaker, elem in chat_rec_jd["questionnaire"].items():
                ...
                for eval_dict in elem["evaluation"]:
                    ...
                    score = self.score_scaling(eval_dict["score"], if_down_scale)
                    #print(score)
                    ...
                    for k in src_dict.keys():
                        ...
                        if src_length >= 256:
                            src_dict[k] = [src_dict[k][0]]+src_dict[k][2:128] + src_dict[k][-128:]
                    # DESCトークン
                    src_dict["input_ids"].append(self.desc_id)
                    src_dict["token_type_ids"].append(0)
                    src_dict["attention_mask"].append(1)
                    # description
                    encode_dict = self.tokenizer(description, add_special_tokens=True)
                    for k in src_dict.keys():
                        tokenized_description = encode_dict[k][1:-1]
